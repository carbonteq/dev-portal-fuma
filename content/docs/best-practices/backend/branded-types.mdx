...
// !info[/\.create\(\)/] Must use .create() method for validation
const userIdResult = UserId.create(123);
const productIdResult = ProductId.create(456);

// !info[/Result<T, Error>/] All creation returns Result type - no exceptions!
...
// !info[/idsResult\.match/] Consistent error handling pattern throughout
idsResult.match({
  Ok: ({userId, productId}) => 
