...
import { Layers, Database, Table, Settings, Cloud } from "lucide-react";
...
Almost every table needs create/updatetimestamps and unique IDs. Instead of copying the same code everywhere, we'll create a reusable "blueprint" (sharedColumns). 
...
export class HostDrizzleRepository extends HostRepository {
  ...
  // !mark(1:13) #22c55e
  update(host: Host): RepositoryEffect<Host, HostUpdateError | HostNotFoundError> {
    return pipe(
...
Fetch many by phone number. We call a reusable helper `fetchMultiple` that runs the query, maps the rows to Host entities, and returns a list.
...
export class HostDrizzleRepository extends HostRepository {
  ...
  // !mark(1:12) #22c55e
  remove(hostId: UUID): RepositoryEffect<boolean, HostDeleteError | HostNotFoundError> {
    return pipe(
...
<Files>
  <Folder name="infra" defaultOpen>    
    <Folder name="repositories" defaultOpen icon={<Layers />}>
      <File name="user.repository.impl.ts" icon={<BiLogoTypescript />} />
      <File name="game.repository.impl.ts" icon={<BiLogoTypescript />} />
      <File name="todo.repository.impl.ts" icon={<BiLogoTypescript />} />
    </Folder>

    <Folder name="database" icon={<Database />}>
      <File name="schema.ts" icon={<BiLogoTypescript />} />
      <Folder name="tables" icon={<Table />}>
        <File name="user.table.ts" icon={<BiLogoTypescript />} />
        <File name="document.table.ts" icon={<BiLogoTypescript />} />
        <File name="..." icon={<BiLogoTypescript />} />
      </Folder>
      <File name="migrations/" icon={<Database />} />
      <File name="seeders/" icon={<Database />} />
      <File name="utils/" icon={<Database />} />
    </Folder>

    <Folder name="services" icon={<Cloud />}>
      <File name="db-health.service.ts" icon={<BiLogoTypescript />} />
      <File name="run-migrations.ts" icon={<BiLogoTypescript />} />
    </Folder>

    <Folder name="config" icon={<Settings />}>
      <File name="env.config.ts" icon={<BiLogoTypescript />} />
      <File name="orm.config.ts" icon={<BiLogoTypescript />} />
